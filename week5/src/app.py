"""Shared Streamlit shell for Week 5 AI Products Suite."""
...
st.set_page_config(page_title="SafeX AI Products Suite · Week 5", page_icon="🚀", layout="wide", initial_sidebar_state="expanded")
...
def render_sidebar() -> str:
    ...
    modules = MODULE_REGISTRY["week5"]
    active_key = st.session_state.get("week5_active_module_key", "chatbot_arsalan")
    if active_key not in modules:
        active_key = "chatbot_arsalan"
    ...
    with st.sidebar:
        st.markdown(
            """
            <div class="app-mark">
                <div class="app-mark__square">5</div>
                <div>
                    <div class="app-mark__name">SafeX Week 5</div>
                    <div class="app-mark__caption">AI Products Suite</div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown('<div class="side-heading">Select Module</div>', unsafe_allow_html=True)
        selected_key = st.radio(
            "Module Selector",
            options=module_keys,
            format_func=lambda k: f"{modules[k]['name']} ({modules[k]['developer']})",
            index=default_index,
            label_visibility="collapsed",
        )
        st.session_state["week5_active_module_key"] = selected_key
        st.session_state["active_module_key"] = selected_key

        mod = modules[selected_key]
        st.markdown('<div class="side-heading">Module Meta</div>', unsafe_allow_html=True)
        st.markdown(f"**Developer:** {mod['developer']}")
        st.markdown(f"**Role:** {mod['role']}")
        st.markdown(f"**Task Track:** {mod['task']}")
        st.markdown(f"**Domain:** {mod['domain']}")
        st.markdown(f"**Status:** `{mod['status']}`")

        if mod.get("deployed_url"):
            st.markdown(f"🌐 [**Live Hosted URL**]({mod['deployed_url']})")
